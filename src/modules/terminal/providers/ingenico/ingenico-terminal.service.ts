...
//
// PaymentRequest.amount is in kopecks and is passed to Purchase() as-is (BPOSLib.h does
// not document the unit; confirmed with a real 5 UAH payment).

// ExchangeStatuses ECR status "ECR works in normal mode" (ECRCommX docs). Other values
// did not change what the terminal shows.
const ECR_STATUS_NORMAL = 2;
...
export class IngenicoTerminalService implements ITerminalProvider, OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (this.heartbeatMs > 0) {
      this.logger.log(`Heartbeat every ${this.heartbeatMs} ms`);
  ...
  private async heartbeat(): Promise<void> {
    ...
    this.pinging = true;
    try {
      const code = await BPOSLib.exchangeStatuses(ECR_STATUS_NORMAL);
